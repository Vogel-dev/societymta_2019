...
--checks the function commands to see if all neccesary parts are filled, sets defaults if not or returns false.
	...
	if (slothbot ~= false) then
		...
		setElementData(slothbot,"AllowFire",true)
		assigncontroller(slothbot)
		giveWeapon(slothbot,tonumber(weapon),99999,true)
		setElementData(slothbot,"BotWeapon",tonumber(weapon))
		if team ~= nil then
			setElementData(slothbot,"BotTeam",team)
		end
		setElementInterior(slothbot,tonumber(interior))
		setElementDimension(slothbot,tonumber(dimension))
		if mode == "waiting" then
			setElementData(slothbot,"status","waiting")
		elseif mode == "following" then
			setElementData(slothbot,"target",modesubject)
			setElementData(slothbot,"status","following")
		elseif mode == "chasing" then
			setElementData(slothbot,"target",modesubject)
			setElementData(slothbot,"status","chasing")
		elseif mode == "guarding" then
			setBotGuard(slothbot,x,y,z)
		else
			setElementData(slothbot,"status","hunting")
		end
